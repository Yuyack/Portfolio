...
function NomiFlowOverlay({ flow, scaleRatio }) {
  ...
  useEffect(() => {
    ...
    overlay.addEventListener("wheel", handleNativeWheel, { passive: false, capture: true });

    return () => overlay.removeEventListener("wheel", handleNativeWheel, { capture: true });
  }, []);
  ...
  return (
    <div
      ...
      style={{
        left: flow.centerX ? "50%" : `${flow.left * scaleRatio}px`,
        top: `${flow.top * scaleRatio}px`,
        width: `${flow.width * scaleRatio}px`,
        height: `${flow.height * scaleRatio}px`,
        transform: flow.centerX ? "translateX(-50%)" : undefined,
      }}
    >
